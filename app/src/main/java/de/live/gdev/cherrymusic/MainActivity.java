...
public class MainActivity extends AppCompatActivity {
    ...
    public void loadWebapp(boolean doLogin) {
        ...
        if (TextUtils.isEmpty(url_s) || url_s.equals("index.php")) {
            ...
            if (doLogin) {
                String postData = "username=" + profile.getUsername() + "&password=" + profile.getPassword() + "&login=login";
                this.webView.postUrl(url_s, EncodingUtils.getBytes(postData, "base64"));
            }
        }
    }

    // Absolutely kill application, don't keep any stream running
    // webView.onPause won't work for this on some devices
    @Override
